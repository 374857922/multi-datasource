...
public class UserDaoImpl implements IUserDao {
    ...
    @Override
    public Object list() {
        StringBuilder sql = new StringBuilder("select user_id, user_name from mcd_user");
        List<User> list = this.jdbcTemplate.query(sql.toString(), new Object[]{}, new RowMapper<User>() {
            @Override
            public User mapRow(ResultSet rs, int rowNum) throws SQLException {
                User user = new User();
                user.setUserId(rs.getString("user_id"));
